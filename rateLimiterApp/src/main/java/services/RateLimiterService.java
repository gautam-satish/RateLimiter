...
public class RateLimiterService {
	...
	public static boolean validateUserRequest(String userId, String api)
		 ...
		 * window is assumed to be scheduled after 23 hours. 3) If both are true, the status is set to
		 * true and the number of requests made for the specific API is increased by 1.
		 * if either of the above condition fails, an exception(LimitException) is
		 * thrown with the appropriate error message. 4) If the user is not present in
		 * the system, UserNotFoundException is thrown.
		 */
